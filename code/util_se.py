...
def _create_gcn_emb(A, x, num_layer, embedding_dim, se_num, batch_size, seq_len, layer_size=[64, 64, 64]):
    ...
    for k in range(num_layer):
        embeddings = tf.matmul(A, x) #[?, 100, 16]
        temp = tf.matmul(embeddings, all_weights['W_gc%d' % k])
        embeddings = tf.nn.leaky_relu(temp + all_weights['B_gc%d' % k])
